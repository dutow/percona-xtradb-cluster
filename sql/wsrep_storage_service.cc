...
class Wsrep_on {
 public:
  Wsrep_on(THD *thd) : m_thd(thd), m_wsrep_on(thd->variables.wsrep_on) {
    thd->variables.wsrep_on = true;
  }
  ~Wsrep_on() { m_thd->variables.wsrep_on = m_wsrep_on; }
...
int Wsrep_storage_service::commit(const wsrep::ws_handle &ws_handle,
  ...
  if (!is_ordered) {
    ...
    m_thd->wsrep_cs().prepare_for_ordering(wsrep::ws_handle(), wsrep::ws_meta(),
                                           false);
...
void Wsrep_storage_service::store_globals() { wsrep_store_threadvars(m_thd); }
